 ...
 * MoveQueue class
 * 
 * Stores all moves returned by the bots for one round and determines the ordering in which they are executed
 * 
...
public class MoveQueue {
	...
	/**
	 * @param move
	 *            : stores move in the correct list
	 */
	...
	/**
	 * Here is determined which player can do the next attackTransfer move. Player to move first each move is chosen
	 * random. Makes sure that if a player has an illegal move, the next legal move is selected.
	 * 
	 * @param moveNr
	 *            : the number of this attackTransfer move
	 * @param previousMovePlayer
	 *            : the player name who last executed a move
	 * @param previousWasIllegal
	 *            : true if the previous move was illegal
	 * @return : the next attackTransfer move from the queue
	 */
	public AttackTransferMove getNextAttackTransferMove(int moveNr, String previousMovePlayer,
			Boolean previousWasIllegal) {
		...
		if (!previousWasIllegal) {
			if (moveNr % 2 == 1 || previousMovePlayer.equals("")) // first move of the two
			{
				...
				double rand = Warlight2.randomGenerator7.nextDouble();
	...
	/**
	 * @param conditionForPlayer1
	 *            : true if the next move should be player1's
	...
	private AttackTransferMove getMove(Boolean conditionForPlayer1) {
		...
		if (!attackTransferMovesP1.isEmpty() && (conditionForPlayer1 || attackTransferMovesP2.isEmpty())) // get
																											// player1's
																											// move
		{
