 ...
 * MapCreator class
 * 
 * Static class that provides methods to create the map and send information about it to the bots
 * 
...
public class MapCreator {
	...
	/**
	 * @param mapString
	 *            : string that represents the map to be created
	...
	/**
	 * Sets up the map. Make every region neutral with 2 armies to start with. Adds wastelands (> 2 armies on a neutral)
	 * if wastelandSize > 0.
	 * 
	 * @param initMap
	 *            : the map object that hasn't been set up yet, i.e. no armies yet
	 * @param wastelandSize
	 *            : the amount of armies that a wasteland contains
	...
	public static Map setupMap(Map initMap, int wastelandSize) {
		...
		if (wastelandSize > 0) {
			int nrOfWastelands = (int) (map.getSuperRegions().size() / 2); // amount of wastelands is half of the amount
																			// of superRegions

			for (int i = 0; i < nrOfWastelands; i++) {
				// double rand = Math.random();
				double rand = Warlight2.randomGenerator1.nextDouble();
	...
	public static int determineMaxRounds(Map map) {
		...
		return (int) Math.max(60, map.getRegions().size() * 2.5); // minimum of 60, otherwise 2.5 times the number of
																	// regions
	}
