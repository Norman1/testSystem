...
public class Processor {
	...
	/**
	 * asks in a ABBAAB fashion where the players would like to start, each superRegion could get one random region that
	 * can be picked, but some randomness decides if it will be less than that
	 */
	public void distributeStartingRegions() {
		...
		for (SuperRegion superRegion : map.getSuperRegions()) {
			...
			if (nrOfRegions > 0) {
				...
				double rand = Warlight2.randomGenerator2.nextDouble();
		...
		while (i < nrOfPicks) {
			...
			if (region == null) { // get random region
				// double rand = Math.random();
				double rand = Warlight2.randomGenerator3.nextDouble();
	...
	/**
	 * Get the amount of picks that the two players can make from the avaiable starting regions minimum of 3 picks each
	 * (= 6 total)
	 * 
	 * @param availablePicks
	...
	private int getAmountOfStartingPicks(int availablePicks) {
		...
		for (int i = 0; i < k; i++) {
			// double rand = Math.random();
			double rand = Warlight2.randomGenerator4.nextDouble();
	 ...
	 *            : bot's output
	 * @param player
	 *            : player who the output belongs to
	 */
	 ...
	 * 
	 * @param atm
	 *            : attackTransferMove to be queued
	 */
	...
	/**
	 * Executes all placeArmies move currently in the queue Moves have already been checked if they are legal Also
	 * stores the moves for the visualizer
	 */
	...
	/**
	 * Executes all attackTransfer moves currently in the queue Does a lot of legality checks and determines whether it
	 * is an attack or a transfer Also stores the moves for the visualizer
	 */
	...
	private int doAttack(AttackTransferMove move) {
		...
		if (fromRegion.getArmies() > 1) {
			...
			{
				// double rand = Math.random();
				double rand = Warlight2.randomGenerator5.nextDouble();
			...
			{
				// double rand = Math.random();
				double rand = Warlight2.randomGenerator6.nextDouble();
	 ...
	 * 
	 * @param player
	 *            : player to send info to
	 * @param regions
	 *            : list of regions
	 * @param beforeDistribution
	 *            : true if we are still in the process of picking armies
	 */
	 ...
	 *            : player to send info to
	 * @param amount
	 *            : the amount of armies the player can pick
	 */
